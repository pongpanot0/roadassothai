  ...
  try {
    const users = await query({
      query: `
        ...
        FROM news 
        WHERE active = '0' 
          AND news_createdate >= DATE_SUB(CURDATE(), INTERVAL 2 MONTH)
...
export async function POST(request) {
  try {
    ...
    const users = await query({
      query: `insert into news (news_createdate,news_desc,news_title,pic_column,news_desc_en,news_title_en)
      values ('${news_createdate}','${news_desc}','${news_title}','${pic_column}','${news_desc_en}','${news_title_en}')
      `,
      values: [],
    });
    if (news_pic !== null) {
      ...
      const newsPath = `./uploads/${users.insertId}`;
      fs.mkdirSync(newsPath, { recursive: true });
      const path = `./uploads/${users.insertId}/${fileName}`;
      const pathPOST = `/uploads/${users.insertId}/${fileName}`;
    ...
    if (news_pic_length > 0) {
      for (let i = 0; i < news_pic_length; i++) {
        ...
        const newIMG = await query({
          query: `INSERT INTO image_news (news_id, img_path, img_index) VALUES (?, ?, ?)`,
          values: [users.insertId, pathPOST, i],
        });
